...
url = f"https://bases.athle.fr/asp.net/liste.aspx?frmpostback=true&frmbase=bilans&frmmode=1&frmespace=0&frmannee={annee}&frmathlerama=&frmfcompetition=&frmfepreuve=&frmepreuve={epreuve}&frmplaces=&frmnationalite=&frmamini=&frmamaxi=&frmsexe={sexe}&frmcategorie={categorie}&frmvent={vent}"
url_final = clic_url(url_final,year_string.get(), correspondance(epreuve_string.get(),dicos.dico_epreuves_outdoor),correspondance(cate_string.get(),dicos.dico_categories),correspondance(sex_string.get(),dicos.dico_sexes))
print("Final A : "+ url_final)
btn = ctk.CTkButton(window, text="Url a créer",
                    command=lambda :print(f"Command button : {clic_url(url_final,year_string.get(), correspondance(epreuve_string.get(),dicos.dico_epreuves_outdoor),correspondance(cate_string.get(),dicos.dico_categories),correspondance(sex_string.get(),dicos.dico_sexes))}"),
                    border_width=2,
                    corner_radius=5)
...
btn_envoi = ctk.CTkButton(window, text="Envoi", command= lambda :extract_data_from_page(clic_url(url_final,year_string.get(), correspondance(epreuve_string.get(),dicos.dico_epreuves_outdoor),correspondance(cate_string.get(),dicos.dico_categories),correspondance(sex_string.get(),dicos.dico_sexes))))
